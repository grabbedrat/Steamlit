...
for cluster in sorted(set(clusterer.labels_)):
    ...
    cluster_items = cluster_items.drop_duplicates(subset='URL', keep='first')
...
def generate_prompts(cluster_id, indent=""):
    ...
    if bookmarks_in_cluster:
        ...
        unique_bookmarks = []
        seen_urls = set()
        for bookmark in bookmarks_in_cluster:
            if bookmark['url'] not in seen_urls:
                unique_bookmarks.append(bookmark)
                seen_urls.add(bookmark['url'])
        
        bookmark_titles = [bookmark["title"] for bookmark in unique_bookmarks]
